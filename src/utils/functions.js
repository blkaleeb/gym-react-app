...
export function generateWorkout(args) {
  ...
  let sets = SCHEMES[scheme].ratio
    ...
    .reduce((acc, curr, index) => {
      const muscleGroupToUse =
        index < arrOfMuscles.length ? arrOfMuscles[index] : arrOfMuscles[index % arrOfMuscles.length];
      return [
        ...acc,
        {
          setType: curr,
  ...
  const genWOD = sets.map(({ setType, muscleGroup }) => {
    ...
    const filteredOppList = Object.keys(setType === 'compound' ? accessoryExercises : compoundExercises).filter(
      (val) => !includedTracker.includes(val)
    );
    ...
    let repsOrDuraction =
      exercises[randomExercise].unit === 'reps'
        ? Math.min(...SCHEMES[scheme].repRanges) +
          Math.floor(
            Math.random() * (Math.max(...SCHEMES[scheme].repRanges) - Math.min(...SCHEMES[scheme].repRanges))
          ) +
